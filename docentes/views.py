...
def asignacion_detail(request, pk):
	asignacion = get_object_or_404(Asignacion.objects.select_related('curso', 'profesor', 'aula', 'horario'), pk=pk)
...
def asignaciones_by_grado(request):
	...
	qs = Asignacion.objects.select_related('curso', 'profesor', 'aula', 'horario')
	if grado:
